...
void __attribute__((noinline)) verifySpace(void); // ZAK (added void)
void __attribute__((noinline)) watchdogConfig(uint8_t x);

static inline void getNch(uint8_t);
static inline void flash_led(uint8_t);
static inline void watchdogReset(void); // ZAK (added void)
...
// ZAK
#if OSCCAL_VAL > -1
const uint8_t __attribute__((section(".osccal"))) osccal = OSCCAL_VAL;
#endif
...
/*
	// ZAK
	// Use an oscilloscope or some kind of frequency counter to measure the output
	// of D6 and tune OSCCAL until it's around 488-490Hz
	DDRD |= _BV(DDD6);
...
*/
  ...
  flash_led(LED_START_FLASHES * 2);
	...
	// ZAK
	// Need to flush out bad data for MCP2221
			       ...
			       uint16_t address, pagelen_t len)
{
    switch (memtype) {
    case 'E': // EEPROM
#if (defined(SUPPORT_EEPROM) && SUPPORT_EEPROM) || defined(BIGBOOT)
// ZAK
//        while(len--) {
